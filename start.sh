...
SO_DIR=$(dirname "$(find llamacpp \( -name 'libllama*.so' -o -name 'libggml*.so' \) 2>/dev/null | head -1)")
export LD_LIBRARY_PATH="$(dirname "$LS"):$SO_DIR:$LD_LIBRARY_PATH"
...
"$LS" -m "$GGUF" -c 8192 -t 4 --host 127.0.0.1 --port 8080 > /tmp/llama.log 2>&1 &
